...
namespace Banchou.Middleware {
    public class StateLogger : MonoBehaviour, IMiddleware<GameState>, IStoreConnector {
        private class Log {
        ...
        public Middleware<GameState> Middleware => (IStore<GameState> store) => (Dispatcher next) => (object action) => {
